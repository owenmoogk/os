import { Directory, globalCommands } from "./directory/directory"


export function commandError(path: Directory, command: string, args: string[], addOutputs: (output: string[]) => void){
	addOutputs([getFullPathname(path)+"> " + command + " " + args.join(" "), `'${command}' is not recognized as a valid command or executable.`])
}

export function argumentError(path: Directory, command: string, args: string[], addOutputs: (output: string[]) => void){
	addOutputs([getFullPathname(path)+"> " + command + " " + args.join(" "), `'${args[0]}' is not recognized as a valid argument to '${command}'. Type '${command}' to see valid usage.`])
}

export function commandHandler(input: string, path: Directory, setPath: (dir: Directory) => void, addOutputs: (output: string[]) => void){
