...
const blurb = "Hey! Welcome to my website. For the *non* CLI version, go to https://owenmoogk.github.io."
...
export var cd: Command = {
	...
	functionCall: (path, setPath, args) => {
		...
		else if (directories.filter(x => x.name == pathInput && x.parent == path).length == 1){
			setPath(directories.filter(x => x.name == pathInput && x.parent == path)[0])
		}
		else {
...
export var help: Command = {
	...
	functionCall: (path, setPath, args) => {
		return (
			`Commands: ${Object.keys({...path.commands, ...globalCommands}).join(", ")}
